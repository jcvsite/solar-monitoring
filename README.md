...
-   **📺 ESP32 Glance Display (Optional):** Companion firmware for the **Cheap Yellow Display** (CYD, 240×320) lives in its own repo: [Solar-monitoring-viewer-esp32](https://github.com/jcvsite/Solar-monitoring-viewer-esp32). Polls `/api/display` over LAN (no cloud); **Settings → ESP32 display** on the web dashboard configures layout, theme, rotation, settings PIN, and OTA; mDNS auto-discovery (`_solar-monitoring._tcp`).
