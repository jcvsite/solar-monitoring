...
-   **📺 ESP32 Glance Display (Optional):** Companion firmware for the **Cheap Yellow Display** (CYD, 240×320) — [Solar-monitoring-viewer-esp32](https://github.com/jcvsite/Solar-monitoring-viewer-esp32) (development copy in [`esp32_display/`](esp32_display/)). Polls `/api/display` over LAN (no cloud); **Settings → ESP32 display** on the web dashboard configures layout, theme, rotation, settings PIN, and OTA; mDNS auto-discovery (`_solar-monitoring._tcp`). LVGL UI with Glance, BMS cell bars, history sparkline, and touch WiFi setup.
