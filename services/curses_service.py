...
class CursesService:
    ...
    def _draw_header(self, data: Dict, cols: int) -> None:
        ...
        title = getattr(self.app_state, "system_title", None) or "Solar Monitoring"
        # Check if update is available and add to header
        if self.app_state.update_available and self.app_state.latest_version:
            header_text = f"{title} v{self.app_state.version} [UPDATE v{self.app_state.latest_version} AVAILABLE] - {now} - Link: "
        else:
            header_text = f"{title} v{self.app_state.version} - {now} - Link: "
